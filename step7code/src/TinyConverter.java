...
public class TinyConverter  { 
    ...
    public void printTinyCode(IRList tempList) {
        for(int j = 0; j < tempList.getSize(); j++) {
            ...
            if(funcCount >= 0)
                paramCount = getParamCount(Listener.funcList.get(funcCount));
            ...
            if(operand1.matches("\\$P\\d+$")) {
                int paramNum = Integer.parseInt(operand1.split("P")[1]);
                operand1 = "$" + Integer.toString(6 + paramCount - paramNum);
            }
            ...
            if(operand2.matches("\\$P\\d+$")) {
                int paramNum = Integer.parseInt(operand2.split("P")[1]);
                operand2 = "$" + Integer.toString(6 + paramCount - paramNum); 
            }
            if(operand2.matches("\\$R$")) {
                operand2 = "$" + Integer.toString(6 + paramCount);
            }
            ...
            if(result.matches("\\$R$")) {
                result = "$" + Integer.toString(6 + paramCount);
            }
